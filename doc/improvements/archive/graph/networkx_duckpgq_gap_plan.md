...
Status: COMPLETE (2026-08-14). FINAL SCOPE CHOSEN: RETAIN duckpgq
        (PGQ MATCH + native pagerank/wcc/local_clustering + shortest_path) AND
        ADD Onager (DuckDB community ext, Apache-2.0) as the engine for the 4
        NetworkX metrics (eigenvector/closeness/betweenness/louvain) + degree.
        NetworkX removed entirely (dep + venv). `make qa` GREEN. SLICE 0 / B
...
## CAPABILITY MATRIX
  ...
  degree_centrality    | SQL (query.py)| YES (collision) | BLOCKED (name clash)
  louvain_community    | NetworkX      | NO  | BLOCKED
  betweenness_centrality | NetworkX    | NO  | BLOCKED
...
## EXPLORATION FINDINGS (2026-08-14, real runs under venv 3.14)
  ...
  => At CURRENT scale NetworkX metrics are NOT a perf problem (all <1.2s).
  Synthetic scaling (gnp p=0.02), ratio per node-doubling:
    louvain x2.8->4.8 (fine) | eigenvector x4.0 (~O(n^2)) |
    closeness x4.0->6.2 (O(V*(V+E)), WORST) | betweenness_approx x6.3->8.1.
  => closeness is the real future-scale risk (minutes at ~50k nodes).
...
## NETWORKX FOOTPRINT / SCIPY
  ...
  Tests: test_graph_algorithms.py, test_integration_perf.py (both now
    importorskip("networkx")).
  pyproject.toml: "networkx" (unpinned soft dep).
  SciPy is NOT required on the production path: algorithms.py uses pure-Python
    variants (eigenvector power-iteration, betweenness Brandes, closeness BFS,
...
## ANSWER TO "MOVE OFF NETWORKX TO DUCKPGQ?"
  ...
  User chose A1: KEEP NetworkX as the documented fallback (soft, scipy-free on
  the prod path; no perf pain at current scale).

SLICE 0 -- UNDERSTAND THE GAPS  [DONE 2026-08-14]
  tests/test_duckpgq_capabilities.py: + test_M5_native_eigenvector,
    + test_M5_native_closeness, + test_M5_native_degrees_collision (all xfail;
    FAIL LOUD when duckpgq unblocks). Verified 8 probes xfail.
  tests/test_integration_perf.py: + test_closeness_scales_reasonably (budget
    <12x/doubling). PASSED.
  tests/run_perf_benchmarks.py (`make perf`): + graph_louvain, graph_betweenness,
    graph_eigenvector (closeness already present). Verified entries valid.
  `make qa` green (1418 passed). `make lint-audit` clean.

SLICE B -- A1 DOC HYGIENE  [DONE 2026-08-14]
  What changed:
    - doc/graph_design.txt sec 3.2: the primary/fallback list now names all
      four missing metrics (louvain/betweenness/closeness/eigenvector) and
      notes degree_centrality is SQL + duckpgq degrees() is name-collided.
    - sec 11 (deps): `requirements.txt` (deleted) -> `pyproject.toml`; networkx
      noted as optional soft dep. Also fixed two other stale `requirements.txt`
      refs (sec 11 risk table, sec ~17 duckdb-pin line).
    - sec 17.2: "duckpgq 1.1.x line" -> 1.5.x (quirks inherited from 1.1.x).
    - sec 17.3/schema-constraints #6: pagerank/wcc/clustering now documented as
      RESOLVED on 1.5.x (was falsely still said "unstable/segfault").
    - sec 17.3 NetworkX-bridge note: names the 4-metric gap, the xfail probes,
      the gap-plan file, and that NX is scipy-free on the prod path.
    - tests/test_integration_perf.py: `import networkx as nx` ->
...
- PARITY (Onager UNWEIGHTED vs NetworkX WEIGHTED) on real graph:
    ...
    Our real edge weights are NEAR-BINARY (only 1.0 / 2.0; 64.7% == 1.0), so
    unweighted vs weighted is already close (NetworkX w vs uw: eig r=0.83,
    close r=0.92, bet r=0.97). => Onager is a near-drop-in for OUR data.

TWO PATHS:
  PATH B (RECOMMENDED) — adopt Onager as the sole graph backend:
    - Replace duckpgq calls (pagerank/wcc/clustering) AND NetworkX calls (the 4
      metrics) with onager_* table functions. Drop `networkx` AND optionally
      `duckpgq` from deps. Needs a BIGINT node-id remap layer (graph uses string
      entity names as node IDs; build int<->name map, run onager, map back) —
      same pattern duckpgq already uses via int PKs.
    - Pros: native + ~40x faster, removes NetworkX entirely, simpler than
      duckpgq, single extension. Cons: centrality becomes unweighted (acceptable
      on our near-binary-weighted data; a behavioral change to watch).
  PATH A (original, now lower priority) — pure-Python reimplement C1-C4 (weighted):
    - Keeps exact weighted semantics; C1 already eval'd (parity r=1.0, faster
      than nx). More code to own/maintain vs a native extension; largely
      redundant given Onager. Keep as fallback if unweighted centrality is
...
### IMPLEMENTATION LOG (2026-08-14, COMPLETE)
  DECISION (user, 2026-08-14): scope = A-ONLY.
    ...
    - ADOPT Onager for eigenvector/closeness/betweenness/louvain + degree_centrality
      (replaces the NetworkX bridge).
    - REMOVE NetworkX entirely (pyproject dep + venv uninstall; tests no longer
      importorskip it).
  WHAT CHANGED:
    - NEW helpers/graph/onager.py: name<->int ID-remap; reads fin.graph_edges as
      plain (src,dst,weight) BIGINT edges; onager_eigenvector/closeness/betweenness/
      degree (L2-normalised eigenvector; degree normalised by (n-1)); onager_louvain
      (returns labels + Python-computed modularity). All 5 take an optional edges=
      param for synthetic test use.
    - helpers/graph/query.py connect(): INSTALL/LOAD onager alongside duckpgq.
    - helpers/graph/algorithms.py: removed `import networkx` + load_graph/nx_*;
      compute() routes the 5 metrics to Onager, pagerank/wcc/clustering to duckpgq;
      CLI prints [via onager]/[via duckpgq]; write_analytics + ranking preserved.
    - pyproject.toml: removed "networkx".
    - Tests: test_integration_perf.py (integration-marked) + run_perf_benchmarks.py
      (make perf) now time Onager via synthetic edge lists (no networkx).
      test_integration_graph_algorithms.py rewritten to compute()/Onager (name-keyed
      over synth_db). New tests/test_onager_capabilities.py positive contract tests
      (undirected, louvain partition valid, small-clique known values, degree range,
      edges= path). test_duckpgq_capabilities.py M5 probes now point at the
      duckpgq-gap (still xfail) with Onager noted as the provider.
  CAVEATS:
    - Onager centrality is UNWEIGHTED (only pagerank honors weights). Acceptable on
      our near-binary-weighted graph (eig r=0.826, closeness r=0.922, betweenness
      r=0.973 vs NetworkX weighted). Drift guard: test_integration_graph_algorithms.py
      asserts Onager closeness/betweenness vs threshold, not networkx.
    - duckpgq pagerank near-uniform (pre-existing quirk, fixed seed, not a regression).
  VERIFICATION: `make qa` GREEN (ruff + ty + deptry + static + pytest -m "not live
...
### NEXT STEP
  ...
  needs discriminative ranks; (c) drop duckpgq entirely later if Onager adds
