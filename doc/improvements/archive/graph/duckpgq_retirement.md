...
working end-to-end on 1.5.5 with zero duckpgq (Phase D validated ad-hoc
in a throwaway 1.5.5 venv; venv bump left to the user).
Date: 2026-08-14 (proposed) / 2026-08-14 (Phase A landed)
Author: agent (session: graph-engine consolidation follow-up)
...
onager included — tracks DuckDB releases.

Empirically verified (throwaway venvs, 2026-08-14):

| duckdb | duckpgq INSTALL | onager INSTALL |
|--------|-----------------|----------------|
| 1.4.3  | OK              | OK             |
| 1.5.4  | OK (pin)        | OK             |
| 1.5.5  | FAIL (HTTP 404) | OK             |

The 2026-08-14 Onager migration already replaced the NetworkX bridge for
eigenvector/closeness/betweenness/degree/louvain. duckpgq is now used for
exactly three native algorithm wrappers plus a set of 1-hop pattern queries
that plain SQL JOINs can express without any property graph. Retiring it
unpins DuckDB, deletes plumbing, and (measured) speeds the three metrics.

## Inventory of every duckpgq touchpoint
...
|---|---|---|---|---|
| pagerank | pagerank(fin_graph, entity, belongsto) | onager_ctr_pagerank(edge list) | duckpgq output is DEGENERATE on this graph (all 1068 companies tied at the teleport floor 0.000471968752 — zero variance; directed PR over company->sector leaves companies with no inflow). Onager (undirected, consistent with its other centralities) yields 33 distinct scores whose tie groups are exactly the WCC component sizes. Strictly more informative; see Phase A results. | 7ms vs 23ms |
| weakly_connected_component | weakly_connected_component(fin_graph, entity, belongsto) | onager_par_components(edge list) | exact partition parity verified post-landing: 42 = 42 components with IDENTICAL membership multisets [88, 75, 74, 69, 65, ...] | 5ms vs 13ms |
| local_clustering_coefficient | local_clustering_coefficient(fin_graph, entity, belongsto) | onager_par_clustering(edge list) | exact: max abs diff 0.0 across all nodes | 6ms vs 11ms |

Note: the duckpgq wrappers join results back to v_company (companies only,
1068 nodes). The onager replacements materialise over the same BelongsTo
projection (1110 nodes including sectors) then filter to companies — the same
output contract. (Decision needed: keep company-only output, or widen to all
entities? See Open Questions.)
...
|---|---|---|
| CREATE PROPERTY GRAPH fin_graph | query.py L960 | delete |
| INSTALL/LOAD duckpgq | query.py L384, L403; snapshot_db.py L347, L374 | delete (keep INSTALL onager) |
| duckpgq_version drift check (P3.4) | query.py L477-543 | keep duckdb_version half only |
| __duckpgq_internal table filtering in snapshots | snapshot_db.py L497-501 | simplifies away entirely |
| app.py graph connection bootstrap | app.py L97-160, L211 | no change beyond the removal of INSTALL duckpgq |

## Benefits

1. DuckDB unpinned -> can track latest (1.5.5+). Removes the
   pyproject.toml "duckdb==1.5.4" pin and the release-train race that
   already forced an emergency pin (2026-08-13, see memory
   duckdb_version_pin...).
2. The three native metrics get 2-3x faster (7ms/5ms/6ms vs 23ms/13ms/11ms).
3. ~120 lines of extension plumbing deleted; snapshot_db simplifies (no
   __duckpgq_internal filtering).
4. shortest_path gains the full vertex sequence in the native path (not
   just cost) — the CTE fallback already provided it, now it becomes the
   primary.
5. One fewer community extension to track (onager stays).

## Costs / risks

1. pagerank values shift slightly (normalisation). Stored values in
   graph_analytics must be recomputed after deploy (algorithms.py --apply);
...
4. snapshot_db.py verification queries (GRAPH_TABLE count) need a plain
   replacement.
5. Contract tests that assert GRAPH_TABLE usage (if any) need updating.
...
- Validated in a throwaway venv with duckdb 1.5.5 + minimal deps:
    ...
    — all green with zero duckpgq. The project venv itself was NOT
...
- [x] make types clean (production gate) — `ty check helpers app.py` green
- [ ] ty check tests advisory clean (0 errors; warnings allowed)
- [ ] make integration (177 tests) green
- [ ] Full unit suite (1417 tests) green
- [x] wcc: 42 components before/after (live graph) — partition membership IDENTICAL
- [x] clustering: max abs diff 0.0 before/after (live graph)
- [x] pagerank: node set identical; duckpgq ranking was degenerate (all-tied), onager differentiates (33 distinct scores, tie groups = component sizes)
- [x] graph_analytics re-apply after deploy (algorithms --apply) — 3 metrics, 1068 company rows each; 42 stale non-company pagerank rows pruned
- [x] snapshot round-trip (create + verify) works without duckpgq — 33 tests green
- [x] DuckDB 1.5.5 verified end-to-end in a throwaway venv (connect, queries, metrics); project venv bump left to user
- [x] doc/graph_design.txt PGQ sections updated
- [x] doc/improvements/completed.md + pending.md updated

## Open questions

1. ANSWERED (Phase A): companies-only kept — the wrappers filter through
   v_company exactly as the duckpgq implementations did; no downstream
   change. graph_analytics rows for the three metrics are company-only
