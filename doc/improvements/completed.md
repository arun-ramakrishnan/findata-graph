...
**Slice 1 — gap-fill & OCR eval (accepted, not promoted):** kept `pdf_local` primary per review — `lite no-ocr` `96.04%` raw / `96.02%` with post is `20.5×` bbox sidecar, not markdown replacement (`avg recall ≥97.5%` gate not met). `liteparse_post` `_filter_running_headers` over-drops on `liteparse` indented plain text (`719` items vs `pdf_local` markdown) `98.26%→91.33%` so not applied; `liteparse_markdown.py` `_looks_like_company_heading` sufficient for OCR pages (`5` headings on `SBI` OCR). `pix2text` on 4 OCR samples `handwritten 7.27s` LaTeX best, `mixed_table 2.69s` exact; on `SBI 28p` `~18s` `0.65s/page` works but `9×` slower than `pdf_local` — not used for non-OCR. Fixes: per-page `page_texts` so `verify_extraction` on 28p lite now `PASS` (`98.5%` doc, `96.7%` lowest) instead of `FAIL` single-page collapse; `MPLBACKEND=agg` forced even when `matplotlib_inline` present (`ValueError` fix). OCR fallback **PASS**: `lite Tesseract` `0.16–0.30s` on `mixed_table 333 chars` exact `100%`, `scanned_benchmark 135 chars` `100%`, `SBI 28p` forced `lite-ocr` `98.5%/96.7%` `PASS`, `auto` `99.9%/94.7%` `PASS` (pdf_local).

**Slice 2 — cutover (OCR fallback + sidecar, no pdf_local replacement):** added `liteparse_engine.py` mirroring `pdf_local.convert` shape (per-page `{markdown:{text,images}, prunedResult:{liteparse:{bbox_items,w,h}}}` + `get_bbox_sidecar()` for `x/y/w/h` RAG grounding). `pdf_conv_md.py` now wires `liteparse_engine.convert` with image sidecar (`pymupdf` `≥150px & 8192B`; `SBI lite` 3 images, `mixed_table` 1 image) and reuses `tmpdir` lifecycle. Deps already in `pyproject.toml`; `pdf_local.py` kept unchanged (deprecation deferred one release). Docs: `markdown_parse.md` PDF→Markdown section documents new `auto` chain + `lite`/`lite-ocr`/`pix2text` flags + `TESSDATA_PREFIX`/`MPLBACKEND`; `local_pdf_engine_trial.md` addendum for 11-doc pool (7+4, 12 tiny `<100MB` samples `828K`, `xinke-wang/OCRDatasets`). `perf_skills.md:9` already covers `liteparse 2.0.0` trial (no change needed).

**Measured (2026-09-01, i5-6500, 11-doc pool):** born-digital `7 Reports/*.pdf` via `auto` all `PASS` `99.7–99.9%` doc coverage (`72–94%` lowest) via `pdf_local`; `lite no-ocr` `0.10s` `20.5×` `96.04%` sidecar `PASS` `98.4%/96.7%` on `SBI`; scanned `4` via `auto` `lite OCR` `0.16–0.30s` `100%` `PASS`; `pix2text` `1–3s` LaTeX `PASS` on formula pages. Tests: `test_liteparse_markdown` 11 + `test_pix2text_markdown` 5 + `test_liteparse_engine` 7 = 23 PASS (72 total across pdf suites).
...
**Status:** EXECUTED 2026-09-02 (S2a–S2d all landed same day as filed; code rode the #194-titled commit)

Scale corpus + embeddings to 100M elements — the S2 slices over S1's incremental corpus. **S2a** `Corpus.iter_notes(fields="frontmatter")` lazy stream — parity `1243/1243`, tracemalloc peak `59.1 MB → ~0 MB`; the A1 hash shakedown exposed a table-global cache-eviction bug (a foreign-root load deleted every other root's rows) — fixed root-scoped, proven on a DB copy; `derive_themes --corpus` adopted `load_shard("Companies")` (`1078/1243` notes, `359`-edge parity). **S2b** `helpers/core/embed_matrix.py` — `memory/embed_matrix.f32` row-major `N×D` fp32, 64B-aligned rows + JSON sidecar; refresh hash-gated per row (`blake2b(embedding, 8)` — embedding-stable; deviation recorded in-proposal); model-tag mismatch ⇒ full rebuild. **S2c** flat exact KNN: numpy `EmbedMatrix.top_k` default (keeps Tier-1 helpers MAX-free) + MAX `FlatKNN` in `Mojo/bench/flat_knn.py` (deptry-excluded dev group) — passing MAX the raw mmap view SEGV'd (DLPack import can copy to a `16 mod 32` mimalloc buffer → `vmovaps`); FlatKNN materializes a resident 64B-aligned copy at init; parity numpy `20/20` vs brute force, MAX `5/5`, vec0 cross-check `50/50`. **S2d** `app._flat_knn_map` is the hybrid's fallback-of-the-fallback (vec0 → flat matrix → page-local Python cosine; degrades, never 500) with a page-subset staleness gate returning None; `rebuild_note_search` refreshes the matrix hash-gated in BOTH branches. Gates: `45` targeted tests; arc-end `make qa` 7/9 (the 2 `compute_root` worktree-environment fails — made worktree-agnostic under #196), `make perf` 21/22 solo, advisory 10/10, lint-audit PASS.
...
**Status:** EXECUTED 2026-09-03 (W1–W9 all landed same day as filed)
...
**Status:** EXECUTED 2026-09-03 (filed and landed same day)
...
**Status:** EXECUTED 2026-09-04 (filed + Phase 0 same day; the §2 trigger ladder and tripwire own the remaining phases — the archived proposal is their record)
...
**Status:** EXECUTED 2026-09-04 (single-purpose fix, no proposal — follow-up to the #204-era backup audit)
...
**Status:** EXECUTED 2026-09-04 (filed + executed same day; trigger = the 2026-09-04 Milky Mist ingest) ·
**Proposal:** `doc/improvements/archive/tooling/maint_runtime_hardening.md`
...
**Status:** EXECUTED 2026-09-05 (filed + executed same day; trigger = the system_overview trial pass + user go) ·
**Proposal:** `doc/improvements/archive/tooling/archify_diagram_pipeline.md`
...
**Status:** EXECUTED 2026-09-05 (filed + executed same day; trigger = JIT/CPU-hotspot evaluation with live `perf_event_paranoid=1` counters) ·
**Proposal:** `doc/improvements/archive/graph/derive_insights_perf.md`
...
**Proposal:** `doc/improvements/archive/tooling/note_section_search.md`
...
**Status:** EXECUTED 2026-09-06 ·
**Proposal:** `doc/improvements/archive/database/embed_full_reembed.md`
...
**Status:** EXECUTED 2026-09-06/07 (filed + implemented 09-06, live smoke + gates 09-07) ·
