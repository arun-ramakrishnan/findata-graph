...
`doc/improvements/archive/graph/duckpgq_retirement.txt`); every algorithm now runs
...
The knowledge graph stores 1,317 entities in SQLite (`memory/research.db`) —
1,068 companies, 42 sectors, 78 sub_sectors, 9 super_sectors, 12 themes,
108 editions —
connected by 5,126 edges across 13 semantic edge types (§4). At ~1k nodes /
~5k edges a server-grade graph DB is overkill; DuckDB gives in-process graph
...
READS (read-only ATTACH per session, sqlite_scanner):
    memory/graph.duckdb (disk cache; schema v9, §8)
      v_node (1209 rows: all 5 entity kinds) + 12 e_* tables
...
Edge types (extensible; TEXT column). Live counts 2026-08-19:

| edge_type | Direction / semantics | sym | rows |
|---|---|---|---|
| `co_mentioned_in` | company ↔ company (newsletter co-mention, derived) | 1 | 1329 |
| `part_of` / `has_company` | company ↔ sector (legacy two-row pair) | – | 1068 each |
| `exposed_to` | company → theme (cross-sector) | 0 | 348 |
| `belongs_to` | sector → super_sector / sub_sector → sector (hierarchy) | 0 | 120 |
| `cited_in` | company/sector → edition (OKF provenance; okf_activation P) | 0 | 1005 |
| `subsidiary_of` | subsidiary → parent | 0 | 63 |
| `jv_with` | company ↔ company (JV) | 1 | 54 |
| `acquired` | acquirer → acquired (temporal; set valid_to) | 0 | 39 |
| `competes_with` | company ↔ company | 1 | 10 |
| `supplier_to` / `customer_of` | company → company (asymmetric, one direction) | 0 | 6 / 1 |
| `same_group` | company ↔ company (promoter group) | 1 | 4 |
...
skewed (`A_Quarter_That_Refuses_To_Behave` = 360/1,005), so analytics
...
2026-07-28; re-ingest from YAML if ever wanted).
...
rows where the historical contract demands (some metrics 1068 rows vs 1209).
...
1209/1068 rows each, link_prediction 236, voterank 10). Node metrics store
...
- **2026-08-14:** duckpgq RETIRED (A-E) — algorithms → Onager, MATCH →
  ...
  retired; duckdb unpinned. `doc/improvements/archive/graph/duckpgq_retirement.txt`.
- **2026-08-14/15:** graph_algos proposal Phases 1-3 — link prediction
  ...
  `doc/improvements/archive/graph/graph_algos.txt`.
