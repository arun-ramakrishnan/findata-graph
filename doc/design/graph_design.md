...
| D17 | GPL-family deps integration-clean; igraph REOPENED as second dyadic engine in the main venv (operator decision 2026-09-26). First-party code relicensed AGPL-3.0-or-later (commit f8ce2bc5, 2026-09-25), voiding D14's pilot-venv split and D16's license axis. Bridge + tests restored verbatim from commit 720e38ff (deleted at acbba048); igraph declared in pyproject. | Lane disposition: Leiden, weighted centralities, weighted shortest path, maxflow/mincut, isomorphism/motifs are SERVED again (dyadic, igraph); HGX keeps the higher-order seat — both seats live. scipy lane stays primary for L1a closeness/harmonic (shipped, exact, zero-dep); igraph digit-identical there adds nothing. D15's consumer-gated wiring discipline stands: prod route promotion (app.py/API/Onager dispatch handover) still needs a real consumer, not wiring ahead of demand. EasyGraph/Graphina verdicts were technical, unaffected |
| D18 | Switching policy for promotion decisions (operator ruling 2026-09-26, scipy_exact_universe S6): when a newer/cheaper engine makes a previously-impossible surface computable, classify by the OLD surface state before choosing default vs flag. (a) NULL/skipped/UNSERVED + maintenance-class cost (fits stamp/advisory lane) → DEFAULT it into the standing pipeline automatically — flags on nothing are unwired value, and defaulting protects no existing behaviour; no proposal round-trip needed, only record the convention. (b) existing sampled/approx GATE output whose exact version exceeds the gate budget or memory envelope → OPT-IN flag, cost documented beside it (S3 chains doctrine). (c) existing served output with a frozen contract and a different universe → SEPARATE surface, never overwrite (apply-refusal pattern). | Acting on (a) is automatic next time — no operator round-trip. Applied in S6: structure scalars + full-walk WF centralities stamped by default (`v_graph_structure`, `v_centrality_closeness_full/harmonic_full`, stamp wall 2.75 s → 27.5 s); exact chains stays opt-in per (b); full-walk never overwrites the 1,734-row contract per (c). Full table: doc/local/perf/graph_scaling.md §Switching policy |
