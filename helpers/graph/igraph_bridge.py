#!/usr/bin/env python3
"""igraph bridge — second dyadic engine beside Onager (restored D17).
...
Licensing: D17 (2026-09-26) — first-party code is AGPL-3.0-or-later, so
GPL-family deps are integration-clean; igraph is a declared dependency in
``pyproject.toml`` and the D14/D16 pilot-venv split is retired. Restored
from commit 720e38ff (deleted at acbba048 under D16). ``igraph`` stays
lazily imported inside the build functions (cheap import, C core loads
only on real runs). Promotion of individual lanes into prod routes stays
consumer-gated per D15/D17.
