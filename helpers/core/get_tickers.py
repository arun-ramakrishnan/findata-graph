...
def _print_financials_section(financials, info):  # noqa: C901
    ...
    if bs is not None and not bs.empty:
        ...
        col_label = _col_label(latest_col)
        for label in ['Total Assets', 'Total Liabilities Net Minority Interest',
                      'Stockholder Equity', 'Cash And Cash Equivalents',
                      'Total Debt']:
    ...
    if cf is not None and not cf.empty:
        ...
        col_label = _col_label(latest_col)
        for label in ['Operating Cash Flow', 'Capital Expenditure',
                      'Free Cash Flow']:
    ...
    if major_holders is not None and not major_holders.empty:
        ...
        for idx, row in major_holders.head(5).iterrows():
            ...
            if 'Value' in row.index:
                ...
                if hasattr(val, 'item'):
                    val = val.item()
                label = str(idx).replace('Percent', ' %').replace('Held', ' held').replace('Count', ' count')
...
def display_ticker(symbol, detailed=False, entities=None, spellfix_conn=None):
    ...
    if not detailed:
        ...
        line = f"{symbol:15s} | {info.get('longName','?'):40s} | {exchange_name:4s} | {currency} | "
    ...
    if comp:
        ...
        _print_holders_section(
            comp.get('major_holders'),
            comp.get('institutional_holders'),
            comp.get('fund_holders')
        )
        _print_recommendations_section(
            comp.get('recommendations'),
            comp.get('recommendations_summary')
        )
...
def main(symbols=None, detailed=False):
    ...
    if symbols:
        ...
        for sym in symbols:
            display_ticker(sym.upper().strip(), detailed=detailed, entities=entities, spellfix_conn=spellfix_conn)
