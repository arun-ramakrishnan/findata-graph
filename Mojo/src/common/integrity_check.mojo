...
def filename_ok(file_path: String) -> Bool:
    ...
    for i in range(stem_len):
        ...
        var alnum = (c >= "a" and c <= "z") or (c >= "A" and c <= "Z") or (
            c >= "0" and c <= "9"
        )
        if i == 0:
            if not alnum:
...
def meaningful_tokens(name: String, lower_fn: PythonObject) raises -> List[String]:
...
def fuzzy_match(ci: List[String], cj: List[String]) -> Bool:
    ...
    if len(ci) >= 2 and len(shared) >= 2 and (
        subset(ci, shared) or subset(cj, shared)
    ):
...
def main() raises:
    ...
    var parity_mode = py_str(Python.evaluate(
    ...
    for i in range(n):
        ...
        if not fp.__bool__():
            if etype == "sub_sector" or etype == "theme" or etype == "institution":
                valid += 1
    ...
    for k in sort_strs(bt_keys):
        bt_canon.append(
            k + ":" + String(bt_total[k]) + "," + String(bt_valid.get(k, 0))
            + "," + String(bt_invalid.get(k, 0)) + ","
            + String(bt_missing.get(k, 0)))
    canon["hdr.by_type"] = join_list(bt_canon, ",")
    var vr = String(round_fn(
        (Float64(valid) / Float64(n)) * 100.0, 2).__str__())
    var cr = String(round_fn(
        ((Float64(n) - Float64(missing_paths)) / Float64(n)) * 100.0, 2
    ).__str__())
    canon["hdr.summary.validation_rate"] = vr
    canon["hdr.summary.coverage_rate"] = cr
    canon["hdr.summary.missing_path_rate"] = String(round_fn(
        (Float64(missing_paths) / Float64(n)) * 100.0, 2).__str__())
    canon["hdr.summary.file_not_found_rate"] = String(round_fn(
        (Float64(not_found) / Float64(n)) * 100.0, 2).__str__())
    canon["hdr.summary.structure_issues_rate"] = String(round_fn(
        (Float64(bad_structure) / Float64(n)) * 100.0, 2).__str__())
    canon["hdr.summary.filename_issues_rate"] = String(round_fn(
        (Float64(bad_filename) / Float64(n)) * 100.0, 2).__str__())
    print("entities+paths : n=", n, " valid=", valid, " invalid=", invalid,
          " missing=", missing_paths,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    rep.append("Entities: " + String(n) + " (valid " + String(valid)
        + ", invalid " + String(invalid) + ") | validation_rate " + vr + "%")
    ...
    var hc_no_po = to_i(conn.execute(
    ...
    var rel_circ = to_i(conn.execute(
    ...
    var bt_src_bad = to_i(conn.execute(
        "SELECT COUNT(*) FROM relations r JOIN entities e ON r.source=e.name "
        "WHERE r.relation_type='belongs_to' AND e.entity_type NOT IN "
        "('sector','sub_sector')").fetchone()[0])
    var bt_tgt_bad = to_i(conn.execute(
        "SELECT COUNT(*) FROM relations r JOIN entities e ON r.target=e.name "
        "WHERE r.relation_type='belongs_to' AND e.entity_type NOT IN "
        "('super_sector','sector')").fetchone()[0])
    var et_src_bad = to_i(conn.execute(
        "SELECT COUNT(*) FROM relations r JOIN entities e ON r.source=e.name "
        "WHERE r.relation_type='exposed_to' AND e.entity_type!='company'"
    ).fetchone()[0])
    var et_tgt_bad = to_i(conn.execute(
        "SELECT COUNT(*) FROM relations r JOIN entities e ON r.target=e.name "
        "WHERE r.relation_type='exposed_to' AND e.entity_type!='theme'"
    ).fetchone()[0])
    var rel_tm = (po_src_bad + po_tgt_bad + hc_src_bad + hc_tgt_bad
        + bt_src_bad + bt_tgt_bad + et_src_bad + et_tgt_bad)
    var rel_err = (rel_unknown + rel_loops + rel_orphan + rel_tm
        + po_no_hc + hc_no_po + rel_circ)
    t1 = perf_counter_ns()
    ...
    print("relations      : total=", rel_total, " errors=", rel_err,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    rep.append("Relations: total=" + String(rel_total) + " errors="
        + String(rel_err))

    # ------------------------------------- 3. count-only table checks
    t0 = perf_counter_ns()
    var et_total = to_i(conn.execute(
        "SELECT COUNT(*) FROM entity_tags").fetchone()[0])
    var et_orphan = to_i(conn.execute(
    ...
    var ev_total = to_i(conn.execute(
        "SELECT COUNT(*) FROM events").fetchone()[0])
    var ev_unknown = to_i(conn.execute(
        "SELECT COUNT(*) FROM events WHERE event_type NOT IN (" + ev_list
        + ")").fetchone()[0])
    var ev_orphan = to_i(conn.execute(
        "SELECT COUNT(*) FROM events WHERE entity NOT IN "
        "(SELECT name FROM entities)").fetchone()[0])
    var ev_badp = to_i(conn.execute(
    ...
    for ci in range(len(count_tables)):
        ...
        var q_total = to_i(conn.execute(
            "SELECT COUNT(*) FROM " + tbl).fetchone()[0])
        var q_orphan = to_i(conn.execute(
            "SELECT COUNT(*) FROM " + tbl + " WHERE entity NOT IN "
            "(SELECT name FROM entities)").fetchone()[0])
        var q_badp = to_i(conn.execute(
            "SELECT COUNT(*) FROM " + tbl
            + " WHERE json_valid(properties) = 0").fetchone()[0])
    ...
    for i in range(live_rows.__len__()):
        live.append(norm_underscore(py_str(live_rows[i][0])) + ">"
    ...
    print("hierarchy      : belongs_to=", h_total, " drift=", drift,
          " errors=", h_err,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    rep.append("Hierarchy: total_belongs_to=" + String(h_total)
        + " taxonomy_drift=" + String(drift) + " errors=" + String(h_err))

    # ------------------------------------------------ 5. normalization
    t0 = perf_counter_ns()
    var nm_missing = to_i(conn.execute(
        "SELECT COUNT(*) FROM entities WHERE normalized_name IS NULL "
        "OR normalized_name = ''").fetchone()[0])
    var dup_rows = conn.execute(
        "SELECT normalized_name, COUNT(*) c FROM entities GROUP BY "
        "normalized_name HAVING c > 1").fetchall()
    ...
    for vi in range(vaults.__len__()):
        ...
        var md_list = Python.evaluate("list("
    ...
    print("normalization  : missing=", nm_missing, " errors=", nm_errors,
          " warnings=", nm_warn,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    rep.append("Normalization: errors=" + String(nm_errors) + " warnings="
        + String(nm_warn))
    ...
    print("names          : dup_groups=", len(dup_groups), " fuzzy_pairs=",
          len(pairs),
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    rep.append("Duplicate tickers: " + String(len(dup_groups))
        + " groups | fuzzy pairs: " + String(len(pairs)))
    ...
    var vw_rows = conn.execute(
        ...
        "GROUP BY edge_type ORDER BY edge_type").fetchall()
    ...
    for i in range(vw_rows.__len__()):
        ...
        vw_canon.append(
            et + ":" + String(tot) + "," + String(wf) + ","
            + String(tot - wf) + "," + String(wt))
    ...
    print("validity_window: warnings=", vw_warn,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    ...
    var sz_rows = conn.execute(
        ...
        "GROUP BY sector_classification ORDER BY n DESC").fetchall()
    var sizes = List[Int]()
    ...
    if sz_count > 0:
        ...
        mean_s = String(round_fn(
            Float64(total) / Float64(sz_count), 1).__str__())
    ...
    var cap_rows = conn.execute(
        ...
        "GROUP BY cap ORDER BY n DESC").fetchall()
    ...
    canon["graph_summary.sector_size"] = (
        String(sz_count) + ":" + String(sz_min) + ":" + String(sz_med)
        + ":" + String(sz_max) + ":" + mean_s)
    ...
    print("graph_summary  : types=", len(ec_canon), " edges=", len(ed_canon),
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    ...
    var mc_rows = conn.execute(
        ...
        "COUNT(*) > 1 ORDER BY entity_name").fetchall()
    ...
    t1 = perf_counter_ns()
    canon["market_cap_conflicts.conflicts"] = join_list(sort_strs(mc_canon), ",")
    canon["market_cap_conflicts.errors"] = String(len(mc_canon))
    print("mcap_conflicts : conflicts=", len(mc_canon),
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    ...
    if not has_meta.__bool__():
        ...
        var gen_row = conn.execute(
            "SELECT value FROM db_meta WHERE key='generation'").fetchone()
        ...
        var uv = to_i(conn.execute(
            "PRAGMA user_version").fetchone()[0])
        var exp_uv = to_i(dbmod.EXPECTED_USER_VERSION)
        if uv != exp_uv:
            dbm_err += 1
            dbm_reasons.append(
                "user_version " + String(uv) + " != expected "
                + String(exp_uv))
        ...
        if not sv_row.__bool__() or sv_val != exp_sv:
            ...
            dbm_reasons.append(
                "db_meta.schema_version " + sv_repr + " != "
    ...
    print("db_meta        : errors=", dbm_err,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")

    # --------------------------------------------------- 11. note_tags
    t0 = perf_counter_ns()
    var trows = conn.execute(
        "SELECT note_path, tag FROM note_tags").fetchall()
    ...
    print("note_tags      : rows=", tn, " stale=", stale,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    ...
    if not file_exists("memory/graph.duckdb"):
        ...
        if cc_sv != cc_expected:
            cc_drift = 1
        var duck_n = dcon.execute(
            "SELECT COUNT(*) FROM v_node").fetchone()
        var lite_n = to_i(conn.execute(
            "SELECT COUNT(*) FROM entities WHERE entity_type IN "
            "('company','sector','super_sector','sub_sector','theme',"
            "'edition','institution')").fetchone()[0])
        var dn = String("None")
        if duck_n.__bool__():
            dn = py_str(duck_n[0])
        if dn != String(lite_n):
        ...
        for i in range(len(e_tables)):
            ...
            var dcnt = dcon.execute(
                "SELECT COUNT(*) FROM " + tbl).fetchone()
            var scnt = 0
            if ge_present.__bool__():
                scnt = to_i(conn.execute(
    ...
    canon["cache_consistency.schema_version"] = String(
        repr_fn(cc_sv).__str__()) if cc_skipped == 0 else String("None")
    canon["cache_consistency.expected_schema_version"] = (
        String(repr_fn(cc_expected).__str__())
        if cc_skipped == 0 else String("None"))
    canon["cache_consistency.schema_version_drift"] = String(
        cc_drift if cc_skipped == 0 else 0)
    canon["cache_consistency.row_mismatches"] = (
        join_list(sort_strs(cc_mm), ",") if cc_skipped == 0 else String(""))
    canon["cache_consistency.errors"] = String(cc_err if cc_skipped == 0 else 0)
    canon["cache_consistency.warnings"] = String(
        0 if cc_skipped == 0 else 1)
    print("cache_reconcile: mismatches=", len(cc_mm), " drift=", cc_drift,
          " elapsed=", Float64(t1 - t0) / 1e6, "ms")
    rep.append("Cache consistency: skipped=" + String(cc_skipped)
        + " schema_drift=" + String(cc_drift) + " row_mismatches="
        + String(len(cc_mm)))
    ...
    for i in range(len(err_keys)):
        ...
        if ev != String("-1"):
            err_total += to_i(ev)
    print("errors (error-severity checks): ", err_total,
          " | total wall=", Float64(t_end - t_all) / 1e6, "ms")
    ...
    rf.write("\n")
    rf.write("Entities: " + String(n) + " (valid " + String(valid)
        + ", invalid " + String(invalid) + ") | validation_rate " + vr
        + "%")
    rf.write("\n")
    rf.write("")
    ...
    rf.write("\n")
    rf.write("total=" + String(rel_total) + " unknown_type="
        + String(rel_unknown) + " self_loops=" + String(rel_loops)
        + " orphaned=" + String(rel_orphan) + " type_mismatch="
        + String(rel_tm) + " part_of_without_has_company="
        + String(po_no_hc) + " has_company_without_part_of="
        + String(hc_no_po) + " circular=" + String(rel_circ)
        + " -> errors=" + String(rel_err))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    rf.write("## ENTITY_TAGS (ERROR-level; gate-failing)")
    rf.write("\n")
    rf.write("total=" + String(et_total) + " orphaned=" + String(et_orphan)
        + " -> errors=" + String(et_orphan))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    rf.write("## NOTE TAGS (ERROR-level; gate-failing)")
    rf.write("\n")
    rf.write("total=" + String(tn) + " stale=" + String(stale)
        + " -> errors=" + String(stale))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    ...
    rf.write("\n")
    rf.write("total=" + String(ev_total) + " unknown_type="
        + String(ev_unknown) + " orphaned=" + String(ev_orphan)
        + " bad_properties=" + String(ev_badp) + " -> errors="
    ...
    rf.write("\n")
    rf.write("total=" + canon.get("quotes.total", String("0")) + " orphaned="
        + canon.get("quotes.orphaned", String("0")) + " bad_properties="
        + canon.get("quotes.bad_properties", String("0")) + " -> errors="
        + String(q_err))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    ...
    rf.write("\n")
    rf.write("total=" + canon.get("company_metrics.total", String("0"))
        + " orphaned=" + canon.get("company_metrics.orphaned", String("0"))
        + " bad_properties="
        + canon.get("company_metrics.bad_properties", String("0"))
        + " -> errors=" + String(cm_err))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    rf.write("## ORPHAN COMPANIES (ERROR-level; gate-failing)")
    rf.write("\n")
    rf.write("total_companies=" + String(oc_total) + " orphan_companies="
        + String(oc_orphan) + " -> errors=" + String(oc_orphan))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    rf.write("## SECTOR HIERARCHY (ERROR-level; gate-failing)")
    rf.write("\n")
    rf.write("total_belongs_to=" + String(h_total) + " sub_sector_orphans="
        + String(h_sub) + " sector_orphans=" + String(h_sec)
        + " super_sector_orphans=" + String(h_ss) + " multi_parent="
        + String(h_multi) + " cycles=" + String(h_cycles)
        + " taxonomy_drift=" + String(drift) + " -> errors="
        + String(h_err))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    ...
    if cc_skipped == 1:
        ...
        rf.write("schema_version=" + cc_sv + " (expected " + cc_expected
            + ") drift=" + String(cc_drift) + " row_mismatches="
            + String(len(cc_mm)) + " -> errors=" + String(cc_err))
        rf.write("\n")
    ...
    rf.write("\n")
    rf.write("missing=" + String(nm_missing) + " duplicates="
        + String(len(dup_canon)) + " bad_format=" + String(len(bad_fmt))
        + " -> errors=" + String(nm_errors))
    rf.write("\n")
    rf.write("file_mismatches=" + String(len(file_mm)) + " orphaned_files="
        + String(len(orphaned)) + " -> warnings=" + String(nm_warn))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    rf.write("## SEMANTIC UNIQUENESS (duplicate tickers; ERROR-level)")
    rf.write("\n")
    rf.write("duplicate_ticker_groups=" + String(len(dup_groups))
        + " -> errors=" + String(len(dup_groups)))
    rf.write("\n")
    rf.write("")
    rf.write("\n")
    rf.write("## FUZZY NAME SIMILARITY (WARNING-level; advisory)")
    rf.write("\n")
    rf.write("fuzzy_duplicate_pairs=" + String(len(pairs)) + " (suppressed: "
        + String(len(SUPPRESSED)) + " triaged pairs)")
    rf.write("\n")
    ...
    rf.write("\n")
    rf.write("warnings=" + String(vw_warn)
    ...
    rf.write("\n")
    rf.write("sector sizes: count=" + String(sz_count) + " min="
        + String(sz_min) + " median=" + String(sz_med) + " max="
        + String(sz_max) + " mean=" + mean_s)
    rf.write("\n")
    ...
    rf.write("\n")
    rf.write("generation=" + String(dbm_generation) + " errors="
        + String(dbm_err))
    rf.write("\n")
