...
def entities():
    ...
    return _sq().execute(
        ...
        "FROM entities ORDER BY entity_type, name").fetchall()
    ...
    try:
        ...
        for (t,) in con.execute("SHOW TABLES").fetchall():
            if t.startswith(("v_", "e_")):
                row = con.execute(
                    f"SELECT COUNT(*) FROM {t}").fetchone()  # noqa: S608
                if row is None:
...
def sqlite_cache_counts():
    ...
    return {"v_node": one("SELECT COUNT(*) FROM entities"),
...
def python_sections(reps=5):
    ...
    out = {"entities_paths": 0.0, "relations": 0.0, "note_tags": 0.0,
           "cache": 0.0}
    for _ in range(reps):
        chk = DatabaseIntegrityChecker(str(REPO / "memory" / "research.db"),
                                       str(REPO))
        t0 = _t.perf_counter()
        ents = chk.get_all_entities()
        for e in ents:
            if not e.get("file_path") and e["entity_type"] in (
                    "sub_sector", "theme", "institution"):
                continue
            chk.validate_file_path(e.get("file_path", ""),
                                   e.get("entity_type", ""))
...
def python_spawn_wall():
    ...
    py = subprocess.run(  # noqa: S603  # repo-local script + venv python
        [_sys.executable, str(REPO / "helpers" / "misc"
                              / "database_integrity_check.py")],
        capture_output=True, text=True, timeout=60, cwd=REPO, check=False)
    return {"python_rc": py.returncode,
            "python_wall": _t.perf_counter() - t0}
...
def python_all_checks() -> dict[str, str]:  # noqa: C901
    ...
    for c in _CHECKS:
        ...
        p = c.name
        if p == "relations":
            for k in ("total", "unknown_type", "self_loops", "orphaned",
                      "type_mismatch", "part_of_without_has_company",
                      "has_company_without_part_of", "belongs_to_endpoint_bad",
                      "exposed_to_endpoint_bad", "circular", "errors"):
                out[f"{p}.{k}"] = str(r[k])
        elif p in ("entity_tags", "note_tags", "events", "quotes",
                   "company_metrics", "orphan_companies", "hierarchy",
                   "db_meta"):
            for k, v in r.items():
            ...
            out[f"{p}.pairs"] = ",".join(
                ...
                for d in sorted(r["fuzzy_duplicate_pairs"],
                                key=lambda d: (d["name_a"], d["name_b"])))
            out[f"{p}.warnings"] = str(r["warnings"])
            out[f"{p}.errors"] = str(r["errors"])
            ...
            out[f"{p}.smallest"] = ",".join(f"{d['sector']}:{d['n']}" for d in r["smallest_sectors"])
