        ...
        for j in range(i + 1, _N):
            if rng.random() < _P:
                ...
                if rng.random() < 0.6:
                    ...
                    edges.append((a, b, "part_of", "2020-01-01", "2021-01-01"))
    return edges
...
def con():
    ...
    dst.executemany(
        ...
        "valid_to, source_ref) VALUES (?, ?, ?, ?, ?, 'seed')",
        _EDGES,
    )
    dst.commit()
    dst.close()
...
def test_shortest_path_matches_bfs_oracle_labelled(con, src, dst, hops):
    ...
    path = gq.shortest_path(con, src, dst, max_hops=hops, edge_label="CompetesWith")
...
def test_shortest_path_matches_bfs_oracle_as_of(con, src, dst, hops, as_of):
    ...
    dist = _oracle_dist(src, dst, adj)
    path = gq.shortest_path(con, src, dst, max_hops=hops, edge_label=None, as_of=as_of)
    if dist is None or dist > hops:
        assert path is None
        return
    assert path is not None
