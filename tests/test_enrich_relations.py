...
# --------------------------------------------------------------------------- #
_GF_FIXTURES = (Path(__file__).resolve().parents[1] / "tests"
...
def _write_gf_report(path, *, issues=(("Srigee DLM", "SRIGEE.NS"),),
                     unlisted=("Veeda Clinical Research",)) -> str:
...
class TestResolveGfTarget:
    ...
    def test_tier1_verified_hit_gf_only(self):
        out = resolve_gf_target(
            "Ajax Engineering", "AJAXENGG.NS", curated=None,
            fetch_fn=_gf_fetch({"AJAXENGG:NSE": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
    ...
    def test_tier1_hit_with_new_stem_is_yahoo_candidate(self):
        ...
        out = resolve_gf_target(
            "Ajax Engineering", "ACLGATI.BO", curated=None,
            fetch_fn=_gf_fetch({"ACLGATI:NSE": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
        assert out.outcome == "resolved gf-only"
    ...
    def test_first_verified_candidate_wins(self):
        ...
        out = resolve_gf_target(
            "Ajax Engineering", "SRIGEE.NS", curated=None,
            fetch_fn=_gf_fetch({"SRIGEE:NSE": _AJAX_PAGE,
                                "SRIGEE:BOM": _SRIGEE_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
        assert out.outcome == "resolved gf-only"
        assert out.slug == "SRIGEE:NSE"

    def test_below_threshold_kept_as_unverified_for_curation(self):
        out = resolve_gf_target(
            "Bosch Limited", "X.NS", curated=None,
            fetch_fn=_gf_fetch({"X:NSE": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
    ...
    def test_curated_hit_trusted_and_serves_unlisted_too(self):
        ...
        out = resolve_gf_target(
            "Ajax Engineering", None, curated=("AJAXENGG:NSE", "gf_only"),
            fetch_fn=_gf_fetch({"AJAXENGG:NSE": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
    ...
    def test_curated_yahoo_mapped_back_kind_propagates(self):
        out = resolve_gf_target(
            ...
            fetch_fn=_gf_fetch({"GATI:NSE": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
...
class TestResolveTier2:
    ...
    def test_srigee_case_resolved_via_bse_scrip(self):
        ...
        out = resolve_gf_target(
            "Srigee DLM", "SRIGEE.NS", curated=None, tier2=True,
            search_fn=self._search([BseMatch("544399", "SRIGEE",
                                             "SRIGEE DLM LTD")]),
            fetch_fn=_gf_fetch({"544399:BOM": _SRIGEE_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
    ...
    def test_tier2_skips_tier1_tried_slugs(self):
        ...
        out = resolve_gf_target(
            "Ajax Engineering", "BOGUS.NS", curated=None, tier2=True,
            search_fn=self._search([BseMatch("544356", "AJAXENGG",
                                             "AJAX ENGINEERING LTD")]),
            fetch_fn=recording_fetch, **self.KW)  # ty: ignore[invalid-argument-type]
    ...
    def test_wrong_company_rows_stay_unverified(self):
        ...
        out = resolve_gf_target(
            "Gati", "ACLGATI.BO", curated=None, tier2=True,
            search_fn=self._search([BseMatch("500219", "JISLJALEQS",
                                             "JAIN IRRIGATION SYSTEMS LTD")]),
            fetch_fn=_gf_fetch({"500219:BOM": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
        assert out.outcome == "unverified"
        assert out.source_tier == 2
        assert out.parsed_name == "AJAX Engineering Ltd"
    ...
    def test_unlisted_entity_resolves_when_listed_after_all(self):
        ...
        out = resolve_gf_target(
            "Ajax Engineering", None, curated=None, tier2=True,
            search_fn=self._search([BseMatch("544356", "AJAXENGG",
                                             "AJAX ENGINEERING LTD")]),
            fetch_fn=_gf_fetch({"AJAXENGG:NSE": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
    ...
    def test_tier1_unverified_evidence_survives_tier2_still_dead(self):
        ...
        out = resolve_gf_target(
            "Bosch Limited", "X.NS", curated=None, tier2=True,
            search_fn=self._search([BseMatch("999999", "ZZZ",
                                             "SOMEONE ELSE LTD")]),
            fetch_fn=_gf_fetch({"X:NSE": _AJAX_PAGE}), **self.KW)  # ty: ignore[invalid-argument-type]
        assert out.outcome == "unverified"
    ...
    def test_search_failure_is_not_fatal(self):
        ...
        out = resolve_gf_target(
            "Srigee DLM", "SRIGEE.NS", curated=None, tier2=True,
            search_fn=failing_search, fetch_fn=_gf_fetch({}), **self.KW)  # ty: ignore[invalid-argument-type]
        assert out.outcome == "still-dead"

    def test_search_no_rows_reports_no_candidates(self):
        # Bare ticker: tier 2 was its only hope and found nothing.
        out = resolve_gf_target(
            "Hanesbrands", "HBI", curated=None, tier2=True,
            search_fn=self._search([]), fetch_fn=_gf_fetch({}), **self.KW)  # ty: ignore[invalid-argument-type]
        assert out.outcome == "no-candidates"
...
class TestRunGoogleFinancePass:
    def test_missing_report_returns_2(self, db, tmp_path):
        rc = run_googlefinance_pass(
            db, report_path=tmp_path / "absent.txt", cache_dir=tmp_path,
            fetch_fn=_gf_fetch({}))
        assert rc == 2
    ...
    def test_include_unlisted_reports_them_as_no_candidates(self, db, tmp_path):
        ...
        rc = run_googlefinance_pass(
            db, include_unlisted=True, report_path=rp, cache_dir=tmp_path,
            fetch_fn=_gf_fetch({}))
        assert rc == 0
        text = rp.read_text(encoding="utf-8")
    ...
    def test_tier2_run_annotates_rows_and_header(self, db, tmp_path):
        ...
        _write_gf_report(rp, issues=(("Srigee DLM", "SRIGEE.NS"),))
        rc = run_googlefinance_pass(
            db, tier2=True, report_path=rp, cache_dir=tmp_path,
            fetch_fn=_gf_fetch({"544399:BOM": _SRIGEE_PAGE}),
            search_fn=lambda q, c: (
                [BseMatch("544399", "SRIGEE", "SRIGEE DLM LTD")], True))
        assert rc == 0
        text = rp.read_text(encoding="utf-8")
        assert "tier2 (BSE name search): on" in text
        assert ("Srigee DLM | resolved yahoo-candidate (544399.BO) [t2]"
                " | 544399:BOM | 1.00" in text)
...
class TestGfApply:
    def test_apply_persists_map_and_metrics_idempotently(self, db, tmp_path):
        ...
        for _ in range(2):  # second run must not duplicate (delete-by-prefix)
            rc = run_googlefinance_pass(
                db, apply_resolutions=True, report_path=rp,
                cache_dir=tmp_path,
        ...
        maps = db.execute(
            "SELECT entity_name, gf_slug, kind, verified_name FROM "
            "entity_gf_map").fetchall()
        assert maps == [("Srigee DLM", "544442:BOM", "gf_only",
                         "Srigee DLM Ltd")]
        rows = db.execute(
            "SELECT metric_label, value_num, unit, source_ref FROM "
            "company_metrics ORDER BY metric_label").fetchall()
        assert len(rows) == 6
        ...
        assert all(r[3].startswith("googlefinance:544442:BOM:")
                   for r in rows)
        # apply summary line in the report
        assert "apply: 1 entity_gf_map rows · 6 company_metrics rows" in \
    ...
    def test_dry_run_writes_no_rows(self, db, tmp_path):
        ...
        assert db.execute(
            "SELECT COUNT(*) FROM entity_gf_map").fetchone()[0] == 0
        assert db.execute(
            "SELECT COUNT(*) FROM company_metrics").fetchone()[0] == 0
    ...
    def test_yahoo_candidate_gets_map_row_but_no_metrics(self, db, tmp_path):
        ...
        _write_gf_report(rp, issues=(("Srigee DLM", "SRIGEE.NS"),))
        rc = run_googlefinance_pass(
            db, tier2=True, apply_resolutions=True, report_path=rp,
            cache_dir=tmp_path,
            fetch_fn=_gf_fetch({"544399:BOM": _SRIGEE_PAGE}),
            search_fn=lambda q, c: (
                [BseMatch("544399", "SRIGEE", "SRIGEE DLM LTD")], True),
        ...
        assert rc == 0
        assert db.execute(
            "SELECT kind FROM entity_gf_map").fetchone()[0] == \
            "yahoo_mapped_back"
        assert db.execute(
            "SELECT COUNT(*) FROM company_metrics").fetchone()[0] == 0
    ...
    def test_section_renders_yahoo_candidate_symbol(self, tmp_path):
        ...
        outcomes = [GfOutcome(
            "Gati", "ACLGATI.BO", "resolved yahoo-candidate", "GATI:NSE",
            0.95, "Gati Ltd", {"pe_ratio": 12.3})]
        append_gf_report_section(rp, outcomes, include_unlisted=False)
        text = rp.read_text(encoding="utf-8")
        assert "[google_finance]" in text
        assert ("Gati | resolved yahoo-candidate (GATI.NS) | GATI:NSE"
                in text)
...
class TestFinnhubPass:
    ...
    def _seed(db, rows):
        ...
        db.executemany(
            "INSERT OR REPLACE INTO entities VALUES (?, ?, ?, 'company')",
            rows)
        db.commit()
    ...
    def test_writeback_discovered_and_verified(self, db, tmp_path):
        ...
        verify = self._verify({"PEL.NS": {
            "longName": "Piramal Enterprises Ltd",
            "industry": "Insurance"}})
        rc = run_finnhub_pass(
            db, report_path=rp,
            lookup_fn=self._lookup([FhMatch("PEL.NS",
                                            "Piramal Enterprises Ltd")]),
            verify_fn=verify,
            fetch_cache=None, **self.KW)  # ty: ignore[invalid-argument-type]
        assert rc == 0
        # dry-run writes nothing
        assert db.execute(
            "SELECT ticker FROM entities WHERE name = 'Piramal Enterprises'"
        ).fetchone()[0] == "PIEIL.NS"
        text = rp.read_text(encoding="utf-8")
        assert "[finnhub]" in text
        assert ("Piramal Enterprises | writeback-candidate"
                " | PIEIL.NS -> PEL.NS" in text)
    ...
    def test_akzo_guard_blocks_foreign_parent(self, db, tmp_path):
        ...
        rc = run_finnhub_pass(
            ...
            fetch_cache=None, **self.KW)  # ty: ignore[invalid-argument-type]  # prove the guard leaked
        assert rc == 0
    ...
    def test_stored_ticker_excluded_from_candidates(self, db, tmp_path):
        ...
        rc = run_finnhub_pass(
            ...
            verify_fn=self._verify({}), fetch_cache=None, **self.KW)  # ty: ignore[invalid-argument-type]
        assert rc == 0
    ...
    def test_name_mismatch_is_unverified(self, db, tmp_path):
        ...
        rc = run_finnhub_pass(
            db, report_path=rp,
            lookup_fn=self._lookup([FhMatch("500219.BO",
                                            "JAIN IRRIGATION SYSTEMS LTD")]),
            verify_fn=self._verify({"500219.BO": {
                "longName": "Jain Irrigation Systems Ltd"}}),
            fetch_cache=None, **self.KW)  # ty: ignore[invalid-argument-type]
        assert rc == 0
    ...
    def test_missing_report_returns_2(self, db, tmp_path):
        rc = run_finnhub_pass(
            db, report_path=tmp_path / "absent.txt",
            lookup_fn=self._lookup([]), verify_fn=self._verify({}),
            fetch_cache=None, **self.KW)  # ty: ignore[invalid-argument-type]
        assert rc == 2

    def test_apply_writes_ticker_frontmatter_and_cache(self, db, tmp_path):
        note = self._note(tmp_path, "Piramal Enterprises",
                          "ticker: PIEIL.NS")
        ...
        info = {"longName": "Piramal Enterprises Ltd",
                "industry": "Insurance"}
        rc = run_finnhub_pass(
            db, dry_run=False, report_path=rp,
    ...
    def test_apply_parity_with_dry_run(self, db, tmp_path):
        ...
        self._seed(db, [("Piramal Enterprises", "PIEIL.NS", None),
                        ("Gati", "ACLGATI.BO", None)])
        rp = tmp_path / "relations_report.txt"
        _write_gf_report(rp, issues=(("Piramal Enterprises", "PIEIL.NS"),
                                     ("Gati", "ACLGATI.BO")))
        kw = dict(
            report_path=rp, fetch_cache=None,
            lookup_fn=lambda q, c: {
                "Piramal Enterprises": ([FhMatch("PEL.NS",
                                                 "Piramal Enterprises")], True),
        ...
        assert (rc1, rc2) == (0, 0)
        assert db.execute(
            "SELECT ticker FROM entities WHERE name = 'Gati'"
        ).fetchone()[0] == "ACLGATI.BO"  # unverified never written
        assert would == 1
...
class TestTerminalClassifications:
    def _seed_status(self, db, name, status, successor=None):
        ...
        db.execute(
            "INSERT INTO entity_ticker_status VALUES (?, ?, ?, 'now')",
            (name, status, successor))
        db.commit()

    _lookup = staticmethod(lambda matches: (
        lambda q, c: (matches, True)))
    _verify = staticmethod(lambda by_ticker: (
        lambda t: by_ticker.get(t)))
    ...
    def test_gf_pass_skips_terminal_and_reports_it(self, db, tmp_path):
        self._seed_status(db, "Akzo Nobel India", "amalgamated",
                          "JSW Paints")
        rp = tmp_path / "relations_report.txt"
        _write_gf_report(rp, issues=(("Srigee DLM", "SRIGEE.NS"),
        ...
        rc = run_googlefinance_pass(
            db, report_path=rp, cache_dir=tmp_path, fetch_fn=fetch_fn,
            search_fn=lambda *_a, **_k: ([], True), delay=0)
        assert rc == 0
        text = rp.read_text(encoding="utf-8")
        # Srigee swept; Akzo never resolved, only classified.
        assert "Srigee DLM |" in text
        assert "Akzo Nobel India |" not in text.split("[terminal]")[0].split(
            "[google_finance]")[1]
    ...
    def test_finnhub_pass_skips_terminal(self, db, tmp_path):
        ...
        rc = run_finnhub_pass(
            db, report_path=rp, lookup_fn=lookup_fn,
            verify_fn=lambda _t: None, fetch_cache=None,
            cache_dir=tmp_path, delay=0)
        assert rc == 0
        text = rp.read_text(encoding="utf-8")
        assert "[finnhub]" in text
    ...
    def test_classify_cli_roundtrip(self, tmp_path, monkeypatch):
        ...
        rc = er.main(["--classify", "Akzo Nobel India", "amalgamated",
                      "JSW", "Paints"])
        assert rc == 0
        conn = sqlite3.connect(dbp)
        row = conn.execute(
            "SELECT status, successor FROM entity_ticker_status "
            "WHERE entity_name = 'Akzo Nobel India'").fetchone()
        conn.close()
    ...
    def test_classify_rejects_unknown_status(self, tmp_path, monkeypatch):
        ...
        dbp = tmp_path / "r.db"
        monkeypatch.setattr(er, "DB_PATH", dbp)
        assert er.main(["--classify", "Some Co", "merged-into",
                        "Someone"]) == 2
    ...
    def test_unclassify_removes_row(self, tmp_path, monkeypatch):
        ...
        conn.execute(
            "INSERT INTO entity_ticker_status VALUES "
            "('X', 'delisted', NULL, 'now')")
        conn.commit()
        conn.close()
        monkeypatch.setattr(er, "DB_PATH", dbp)
        ...
        conn = sqlite3.connect(dbp)
        n = conn.execute("SELECT COUNT(*) FROM entity_ticker_status"
                         ).fetchone()[0]
        conn.close()
        assert n == 0
    ...
    def test_ticker_ownership_guard_blocks_sibling(self, db, tmp_path):
        ...
        rp = tmp_path / "relations_report.txt"
        _write_gf_report(rp,
                         issues=(("Kotak Mahindra Life Insurance",
                                  "KOTAKLIFE.NS"),))
        rc = run_finnhub_pass(
            db, report_path=rp,
            lookup_fn=self._lookup([FhMatch("KOTAKBANK.NS",
                                            "Kotak Mahindra Bank Limited")]),
            verify_fn=self._verify({
                "KOTAKBANK.NS": {
                    "longName": "Kotak Mahindra Bank Limited"}}),
            fetch_cache=None, cache_dir=tmp_path, delay=0)
        assert rc == 0
        text = rp.read_text(encoding="utf-8")
        ...
        assert db.execute(
            "SELECT ticker FROM entities "
            "WHERE name = 'Kotak Mahindra Life Insurance'"
        ).fetchone()[0] == "KOTAKLIFE.NS"
    ...
    def test_stale_report_target_skipped(self, db, tmp_path):
        ...
        self._seed_entities(db, [("Tata Motors Passenger Vehicles",
                                  "TMPV.NS", None)])
        rp = tmp_path / "relations_report.txt"
        _write_gf_report(rp, issues=(
        ...
        def lookup_fn(query, _cache_dir):
            probed.append(query)
            return [FhMatch("TMPV.NS",
                            "Tata Motors Passenger Vehicles Limited")], True
