...
def test_full_graph_smoke_pr_louvain_leiden_timing():
    ...
    # 57,632 / 22,054 after the VIGIL corporate intake, 2026-09-23).
    assert 19_000 <= len(rows) <= 100_000, f"unexpected edge count {len(rows)}"
    t0 = time.perf_counter()
    g, _ = B.build_graph(rows)
    t_build = time.perf_counter() - t0
    assert 1_600 <= g.vcount() <= 26_000, f"unexpected node count {g.vcount()}"
