...
def test_resolve_edition_string_variant_forms(tmp_path):
    ...
    assert resolve_edition_string("The Chatter — Note Alpha", index) == (
        chatter / "Note_Alpha.md")
    assert resolve_edition_string("Note Alpha, Edition #3", index) == (
        chatter / "Note_Alpha.md")
    assert resolve_edition_string("Note Alpha, Zerodha", index) == (
        chatter / "Note_Alpha.md")
    assert resolve_edition_string("Note Alpha, Aug 2026", index) == (
        chatter / "Note_Alpha.md")
    # containment fallback for a long candidate against a shorter key
    assert resolve_edition_string(
        "Plain Stem special extended weekend edition digest", index) == (
