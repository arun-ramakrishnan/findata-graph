...
class TestParseBseResponse:
    def test_exact_hit_row(self):
        ...
        assert matches == [BseMatch(scrip="544399", symbol="SRIGEE",
                                    name="SRIGEE DLM LTD",
                                    isin="INE0RJ901010")]

    def test_tmpv_row_carries_nse_symbol(self):
        # BSE rows carry the NSE symbol (TMPV) — the G4 writeback case.
        matches = parse_bse_response(
            _decode_body(_load("bse_tata_motors_pas.json")))
    ...
    def test_no_match_found_is_empty(self):
        assert parse_bse_response(
            _decode_body(_load("bse_zzznosuchco.json"))) == []
...
class TestBseSearchCached:
    ...
    def test_cache_miss_fetches_and_writes(self, tmp_path, monkeypatch):
        monkeypatch.setattr(es, "bse_search",
                            lambda query, **_kw:
                            _decode_body(_load("bse_srigee.json")))
        matches, from_cache = bse_search_cached("Srigee DLM", tmp_path)
        ...
        assert parse_bse_response(
            cache_file.read_text(encoding="utf-8")) == matches
